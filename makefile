...
ARPACKPP_DIR = $(HOME)/Documents/cuda_ensemble/methods_fitting_cuda/ClusterEnsembleCreateMyAggloSeqAr/external/arpack++
...
INC := -Iinc -Iexternal -I$(MABLAB)/extern/include -I$(ARPACKPP_DIR)/include -I$(SUITESPARSE_DIR_LOCAL)/CHOLMOD/Include -I$(SUITESPARSE_DIR_LOCAL)/SuiteSparse_config
LIBS := -L$(MATLAB_INSTALL_PATH)/bin/glnxa64 -L$(LAPACK_PATH) -L$(OPENBLAS_PATH) -L$(ARPACKPP_LIB_DIR)
#-L$(ARPACKPP_DIR)/external

#put non cuda -l* stuff here
NONCU_LIBS:=-larpack -lgfortran -lblas
#-lsuperlu -llapack 
CC = gcc
CXX = g++
MEX_EXE=mex
